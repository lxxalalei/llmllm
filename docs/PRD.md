...
L4 的交付单位是一个用户能独立理解的问题：包含直接结论、适用条件和已知结果；有证据时提供可执行操作。同义问法归入同一知识的 `question_variants`，不另建 FAQ。必要条件不得写成成功保证，不能用“由相关配置决定”等循环解释代替有效答案。

模型必须先提出新建、合并或缺证据的计划，再基于选定 Rule 写作和审核。代码固定知识层级、证据范围、权限及草稿状态；模型审核通过不等于发布。操作路径、当前配置值等无法由已有 Rule 支持的内容进入缺口记录。运行时标准见 [L4 写作规范](../app/llm/prompts/l4_authoring.md)，使用与验收见 [L4 用户意图工作流](l4-user-intents.md)。
